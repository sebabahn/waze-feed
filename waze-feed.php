...
<incidents>
    ...
    $positionsEndpoint = $traccarUrl . '/api/positions?limit=-1';
    $ch = curl_init();
    curl_setopt_array($ch, [
    ...
    foreach ($devices as $device) {
        ...
        // Prüfen, ob emstatus-Filter aktiv ist
        if (!empty($config['EMSTATUS_FILTER'])) {
            $emstatus = $device['attributes']['emstatus'] ?? null;
            if ($emstatus === null || !in_array((int)$emstatus, $config['EMSTATUS_FILTER'])) {
                logDebug("Device {$deviceId} (emstatus: {$emstatus}) nicht in EMSTATUS_FILTER. Übersprungen.");
                continue;
            }
        }
        ...
        // Koordinaten auf 4 Nachkommastellen runden für XML-Ausgabe
        $lat = round($lat, 4);
        $lon = round($lon, 4);
